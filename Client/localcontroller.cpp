...
LocalController::LocalController(QObject *parent, MainWindow *mw, Protocol *p) : QObject(parent), m_mainWindow(mw), m_protocol(p)
{
    ...
    connect(m_protocol->socket(), &QTcpSocket::disconnected, this, &LocalController::disconnectFromHost);
...
void LocalController::initGuiController()
{
    ...
    connect(m_mainWindow->connectAction()->menu()->actions().at(1), &QAction::triggered, this, &LocalController::disconnectFromHost);
...
void LocalController::removeUser(const QString &nickname)
{
    ...
    m_mainWindow->tabBar()->removeTab(findTabIndex(nickname));
    if(m_mainWindow->listWidget()->currentItem() == item)
...
void LocalController::sendMessage(const QString &message)
{
    ...
    if(m_serverInfoItem != nullptr && m_serverInfoItem->isNickNameValid() == false)
    {
        ...
        else
        {
            ...
            m_mainWindow->chat()->setHtml(m_serverInfoItem->message());
        }
    }
...
void LocalController::initServerInfoItem()
{
    ...
    m_serverInfoItem->appendMessage(NEW_LINE);
    m_serverInfoItem->appendMessage("Set your nickname in first message");
...
void LocalController::disconnectFromHost()
{
    ...
    disconnect(m_protocol, &Protocol::globalMessage, this, &LocalController::displayGlobalMessage);
    disconnect(m_protocol, &Protocol::privateMessage, this, &LocalController::displayPrivateMessage);
...
void LocalController::signalNewMessage(const QString &tabName)
{
    ...
    if(itemIdx != lw->currentRow())
    {
        ...
        QTabBar *tb = m_mainWindow->tabBar();
        tb->setTabTextColor(tb->currentIndex(), QColor(255, 0, 0));
